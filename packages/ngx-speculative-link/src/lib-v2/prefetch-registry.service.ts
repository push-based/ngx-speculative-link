...
export class PrefetchRegistry {
  ...
  preResolve(treeOrUrl: UrlTree | string) {
    const url =
      typeof treeOrUrl === 'string'
        ? treeOrUrl
        : this.#router.serializeUrl(treeOrUrl);
    if (this.#routeToPreResolve.has(url)) {
  ...
  #setLinkPreResolver(tree: UrlTree) {
    ...
    for (const route of this.#preResolveRoutes) {
      ...
      const matchResult = matchingSegmentGroups(
        tree.root,
        preResolveRouteTree.root,
        preResolveRouteTree.root.segments
      );
      if (matchResult) {
        preResolveFns.push(() =>
          route.data!['preResolve']({
            data: route.data,
            params: matchResultToParams(matchResult),
          })
        );
      }
    }
  ...
  #getMatches(
    ...
    return uniqBy(
      ...
      .filter(
        (match): match is { url: string; params: Params } => match !== null
      );
  }
}
